...
import { Rectangle, Line, Point, Color } from "./Rectangle";
...
function setupRectangles() {
  ...
  for (let i = 0; i <= columnsCount; i++) {
    for (let j = 0; j <= cellsCount; j++) {
      ...
      const createRect = includeOuterEdges || (i < columnsCount && j < cellsCount);
      ...
      if (createRect) {
        ...
        if (i < columnsCount && j > 0) {
          ...
          const x = i >= columnsCount ? maxX : (i < columnsCount ? columns[i].cells[0].centerX : minX);
          const y = j === 0 ? minY : (j > 0 ? columns[0].cells[j-1].centerY : maxY);
          topRight = { x, y };
        }
        ...
        if (i < columnsCount && j < cellsCount) {
          ...
          // Create virtual point
          const x = i >= columnsCount ? maxX : (i < columnsCount ? columns[i].cells[0].centerX : minX);
          const y = j >= cellsCount ? maxY : (j < cellsCount ? columns[0].cells[j].centerY : minY);
          bottomRight = { x, y };
        }
        ...
        rect.setMetadata({
          ...
          colBottomRight: i,
          rowBottomRight: j,
          isEdgeRect: i === 0 || j === 0 || i === columnsCount || j === cellsCount
        });
...
function updateRectangles() {
  ...
  for (const rect of rectangles) {
    ...
    if (metadata) {
      const { colTopLeft, rowTopLeft, colBottomRight, rowBottomRight, isEdgeRect } = metadata;
      ...
      // Top-left point
      if (colTopLeft >= 0 && rowTopLeft >= 0 && colTopLeft < columnsCount && rowTopLeft < cellsCount) {
        topLeft = columns[colTopLeft].cells[rowTopLeft].center;
      } else {
        // Virtual point
        const x = colTopLeft < 0 ? minX : (colTopLeft >= columnsCount ? maxX : columns[colTopLeft].cells[0].centerX);
        const y = rowTopLeft < 0 ? minY : (rowTopLeft >= cellsCount ? maxY : columns[0].cells[rowTopLeft].centerY);
        topLeft = { x, y };
      }
      
      // Top-right point
      if (colBottomRight >= 0 && rowTopLeft >= 0 && colBottomRight < columnsCount && rowTopLeft < cellsCount) {
        topRight = columns[colBottomRight].cells[rowTopLeft].center;
      } else {
        // Virtual point
        const x = colBottomRight < 0 ? minX : (colBottomRight >= columnsCount ? maxX : columns[colBottomRight].cells[0].centerX);
        const y = rowTopLeft < 0 ? minY : (rowTopLeft >= cellsCount ? maxY : columns[0].cells[rowTopLeft].centerY);
        topRight = { x, y };
      }
      
      // Bottom-right point
      if (colBottomRight >= 0 && rowBottomRight >= 0 && colBottomRight < columnsCount && rowBottomRight < cellsCount) {
        bottomRight = columns[colBottomRight].cells[rowBottomRight].center;
      } else {
        // Virtual point
        const x = colBottomRight < 0 ? minX : (colBottomRight >= columnsCount ? maxX : columns[colBottomRight].cells[0].centerX);
        const y = rowBottomRight < 0 ? minY : (rowBottomRight >= cellsCount ? maxY : columns[0].cells[rowBottomRight].centerY);
        bottomRight = { x, y };
      }
      
      // Bottom-left point
      if (colTopLeft >= 0 && rowBottomRight >= 0 && colTopLeft < columnsCount && rowBottomRight < cellsCount) {
...
const animation: AnimationFunction = (p: p5, normalizedTime: number): void => {
  ...
  for (const rect of rectangles) {
    ...
    const color = rect.getColor();
    
    // Draw filled polygon
    p.fill(color.r, color.g, color.b, color.a * 255); // p5 uses 0-255 for alpha
    p.noStroke();
