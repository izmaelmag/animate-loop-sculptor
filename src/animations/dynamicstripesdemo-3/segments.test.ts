...
describe("dynamic stripes marquee segments", () => {
  ...
  it("preserves stable logical owner index across seam splits", () => {
    const pieces = splitSegmentsToUnitIntervalWithOwners([
      [0.1, 0.3],
      [0.92, 1.08],
      [1.2, 1.35],
    ]);
    ...
    expect(pieces[0].ownerIndex).toBe(0);
    expect(pieces[1].ownerIndex).toBe(1);
    expect(pieces[2].ownerIndex).toBe(1);
    expect(pieces[3].ownerIndex).toBe(2);
  });
