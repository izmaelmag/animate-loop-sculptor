...
// MODIFIED: useClickOutside hook now expects an array of refs to ignore
...
const ColorPalette: React.FC<ColorPaletteProps> = ({ 
  ...
  const currentTriggerRef = editingIndex !== null ? triggerRefs.current[editingIndex] : null;
  ...
  useClickOutside(
      [popoverRef, triggerRefs.current[editingIndex!] as React.RefObject<HTMLElement>].filter(Boolean), // Pass both refs, filter nulls
      () => {
  ...
  const displayColors = colors.slice(0, 12).concat(Array(Math.max(0, 12 - colors.length)).fill(null));
  
  return (
    <div className={`relative p-2 rounded-md ${isEditing ? 'bg-gray-700/50 border border-yellow-500/50' : ''} ${className}`}> {/* Added padding/border in edit mode */}
      <div className="flex justify-between items-center mb-2"> 
         {/* Make edit state text more prominent */}
         <span className={`text-xs font-medium ${isEditing ? 'text-yellow-400' : 'text-gray-400'}`}> 
             {isEditing ? 'Editing Palette' : 'Select Color'}
         </span>
         <button 
             onClick={() => setIsEditing(!isEditing)}
             className={`p-1 rounded ${isEditing ? 'text-yellow-300 hover:bg-yellow-900/50' : 'text-gray-400 hover:bg-gray-700 hover:text-gray-200'}`}
        ...
        {displayColors.map((color, index) => (
          <div
            ...
            className={`w-8 h-8 rounded border 
                       ${color ? 'cursor-pointer' : 'bg-gray-700 cursor-not-allowed'}
                       ${isEditing && color ? 'border-yellow-600 hover:ring-2 hover:ring-offset-1 hover:ring-offset-gray-700 hover:ring-yellow-400' : 'border-gray-600'} 
                       ${!isEditing && color ? 'hover:ring-2 hover:ring-offset-1 hover:ring-offset-gray-800 hover:ring-blue-400' : ''}
                       ${editingIndex === index ? 'ring-2 ring-offset-1 ring-offset-gray-700 ring-yellow-500' : ''} `} 
            style={color ? { backgroundColor: color } : {}} 
            title={isEditing ? (color ? `Edit: ${color}` : 'Empty') : (color || 'Empty slot')} 
          />
        ))}
      </div>
